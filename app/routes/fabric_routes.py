...
def cuts_history():
    ...
    # --- базовый запрос по срезам + фабрика пользователя ---
    ...
    # простой текстовый поиск по названию ткани
    if q:
        like = f"%{q.lower()}%"
        q_base = q_base.filter(db.func.lower(Fabric.name).like(like))

    # --- данные для статистики (без пагинации) ---
    q_stats = q_base
    all_cuts = q_stats.all()

    # --- сортировка для таблицы / пагинации ---
    q_ordered = q_base
    if sort == "date_asc":
    ...
    # --- расчёт статистики для карточек ---
    totals_by_unit = {}
    for c in all_cuts:
    ...
    # есть ли в данных поля остатка/комментария (на будущее)
    ...
    # список тканей для фильтра (select)
    fabric_options = (
        Fabric.query
    ...
    # порог для "больших" списаний (подсветка)
