...
def alerts(update: Update, context: CallbackContext) -> None:
    ...
    txt = (
        "📊 <b>Краткий обзор</b>\n\n"
        f"• Товаров с низким остатком в магазине (< {LOW_STOCK_THRESHOLD} шт.): "
        f"<b>{low_count}</b>\n"
        f"• Заказов магазина в статусе 'pending': <b>{pending_count}</b>\n"
        f"• Последнее движение по складу: <b>{last_mv_str}</b>\n\n"
        "Подробнее:\n"
...
def product_cmd(update: Update, context: CallbackContext) -> None:
    ...
    if pid:
        product = base_q.filter(Product.id == pid).first()

    if not product:
        product = base_q.filter(Product.name.ilike(f"%{query_str}%")).first()

    if not product:
        update.message.reply_text("Товар не найден.")
        return

    # Factory qty (your Product.quantity is фабричный склад)
    ...
    txt = (
        f"📦 <b>{product.name}</b>\n"
        f"ID: <code>MM-{product.id:05d}</code>\n"
