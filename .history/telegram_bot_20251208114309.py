...
def product_detail_callback(update: Update, context: CallbackContext) -> None:
    """
    Обработчик нажатия на кнопку с product (callback_data = 'prod:<id>').
    ...
    # ВАЖНО: для БД нужен app_context
    with flask_app.app_context():
        product = (
            Product.query
    ...
    # ответить на callback (убирает "часики") и заменить текст сообщения
    query.answer()
