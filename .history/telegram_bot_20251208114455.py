...
# ------------------------------------------------------------------------------
# ------------------------------------------------------------------------------
# /product [text] – list products with inline buttons
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# Callback: product_detail_callback – show details when button is pressed
# ------------------------------------------------------------------------------

def product_detail_callback(update: Update, context: CallbackContext) -> None:
    """
    Обработчик нажатия на кнопку товара (callback_data = 'prod:<id>').
    Показывает подробную карточку товара.
    """
    ...
    # Нужен Flask app_context для работы с БД
    with flask_app.app_context():
        product = (
            Product.query
            .filter_by(id=product_id, factory_id=DEFAULT_FACTORY_ID)
            .first()
        )
    ...
    # ответить на callback (убрать "часики") и заменить текст
    query.answer()
