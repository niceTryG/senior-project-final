...
def shop_low(update: Update, context: CallbackContext) -> None:
    ...
    # Do ALL DB and ORM access inside app_context, build plain strings
    with flask_app.app_context():
        ...
        if not low_items:
            # we can reply directly here too
        ...
        for row in low_items:
            p: Product = row.product  # ORM access is still inside context
            lines.append(
    ...
    # Outside of context we only use the ready-made text
    update.message.reply_text("\n".join(lines))
...
def shop_stock(update: Update, context: CallbackContext) -> None:
    ...
    # Do ALL DB work + ORM attribute access INSIDE context
    with flask_app.app_context():
        query = (
            db.session.query(ShopStock)
            .join(Product)
