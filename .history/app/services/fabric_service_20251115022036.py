...
class FabricService:
    ...
    # -------- LIST / SEARCH --------
    # -------- LIST / SEARCH --------
    def search_fabrics(self, query: str | None, sort: str = "name", category: str | None = None):
        q = Fabric.query
    ...
    def get_usage_summary(self, date_from=None, date_to=None):
        """
        Отчёт по использованию ткани за период.
        Возвращает dict:
          - rows: список {fabric: Fabric, total_used: float}
          - total_used: общий расход (сумма, без деления по kg/m)
        """
        q = Cut.query.join(Fabric)
