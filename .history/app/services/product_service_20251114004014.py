...
class ProductService:
    ...
    def get_manager_financial_report(self):
        from ..models import Product, Fabric, Sale
        from sqlalchemy import func
        from datetime import datetime

        # --- 1. FABRICITY STOCK COST ---
        factory_cost = db.session.query(
            func.sum(Product.quantity_factory * Product.cost_price)
        ).scalar() or 0

        # --- 2. SHOP STOCK SELL PRICE ---
        shop_value = db.session.query(
            func.sum(Product.quantity_shop * Product.sell_price)
        ).scalar() or 0

        # --- 3. FABRIC VALUE ---
        fabric_value = db.session.query(
            func.sum(Fabric.quantity * Fabric.price_uzs)
        ).scalar() or 0

        # --- 4. TODAY SALES ---
        today = datetime.utcnow().date()
        today_sales = (
            db.session.query(func.sum(Sale.total_price_uzs))
        ...
        # --- 6. MONTH COSTS & PROFIT ---
        month_costs = (
            db.session.query(func.sum(Sale.quantity * Product.cost_price))
        ...
        month_profit = month_sales - month_costs

        # --- 7. LOW STOCK ---
        low_stock = Product.query.filter(Product.quantity_shop + Product.quantity_factory < 5).all()

        # --- 8. PER PRODUCT DETAILS TABLE ---
        products = []
        all_products = Product.query.all()
        for p in all_products:
            total_qty = (p.quantity_factory or 0) + (p.quantity_shop or 0)
            potential_profit = total_qty * (p.sell_price - p.cost_price)
            ...
            realized_profit = sold_units * (p.sell_price - p.cost_price)

            products.append({
                "name": p.name,
                "factory_qty": p.quantity_factory or 0,
                "shop_qty": p.quantity_shop or 0,
        ...
        # --- RETURN DATA ---
        return {
            "factory_cost_uzs": factory_cost,
            "shop_sell_uzs": shop_value,
            "fabric_value_uzs": fabric_value,
            "today_sales_uzs": today_sales,
            "month_sales_uzs": month_sales,
            "month_profit_uzs": month_profit,
            "low_stock": low_stock,
            "products": products,
        }
