...
class ProductService:
    ...
    def get_manager_financial_report(self):
        """Complete financial overview for manager (Dad)."""

        products = Product.query.all()
        sales = Sale.query.all()

        # REALIZED PROFIT (money earned from actual sales)
        realized_profit = sum(s.total_sell - s.total_cost for s in sales)

        # STOCK (FACTORY — cost basis)
        factory_cost_uzs, factory_cost_usd = self.total_stock_value()

        # STOCK (SHOP — sell basis)
        shop_sell_uzs, shop_sell_usd = self.shop_stock_totals()

        # UNREALIZED PROFIT (future profit if all sells)
        ...
        for p in products:
            ...
            product_list.append({
                "name": p.name,
                "factory_qty": p.quantity,
                "shop_qty": shop_qty,
                "cost_price": p.cost_price_per_item,
                "sell_price": p.sell_price_per_item,
                "potential_profit": p.stock_profit(),
                "sold_units": sum(s.quantity for s in p.sales),
                "realized_profit": sum((s.total_sell - s.total_cost) for s in p.sales),
            })

        return {
            "factory_cost_uzs": factory_cost_uzs,
            "shop_sell_uzs": shop_sell_uzs,
            "total_investment": factory_cost_uzs,
            "total_value_if_sold": shop_sell_uzs,
