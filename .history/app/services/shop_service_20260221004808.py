...
class ShopService:
   ...
   def export_items_csv(
       ...
       """
       Экспорт склада магазина в CSV (Excel-friendly).

       Важно:
       - Excel на Windows часто открывает CSV в неверной кодировке при двойном клике.
       - Чтобы кириллица всегда отображалась корректно без "Ð…", отдаём файл в UTF-16LE с BOM.
       - Также добавляем первую строку `sep=,`, чтобы Excel правильно разбивал колонки по запятой.
       """
       data = self.list_items(q=q, sort=sort, factory_id=factory_id)
       items = data["items"]

       # csv module expects newline="" for correct 
handling; StringIO works fine
       output = io.StringIO()
       writer = csv.writer(output, delimiter=",", lineterminator="\r\n")

       # Hint for Excel delimiter
       writer.writerow(["sep=,"])
       ...
       # UTF-16LE with BOM (Excel-friendly)
