...
def _import_folder(factory_id: int) -> str:
    base = current_app.config.get("EXCEL_IMPORT_FOLDER") or os.path.join("app", "static", "uploads", "excel_imports")
    folder = os.path.join(base, str(factory_id))
...
def import_upload():
    ...
    folder = _import_folder(factory_id)
...
@login_required
@roles_required("admin", "manager")
def import_batch_download(batch_id: int):
    factory_id = _ensure_factory_bound()
    if factory_id is None:
        return redirect(url_for("products.import_wizard"))

    batch = ExcelImportBatch.query.filter_by(id=batch_id, factory_id=factory_id).first_or_404()
    return send_file(batch.stored_path, as_attachment=True, download_name=batch.filename)
