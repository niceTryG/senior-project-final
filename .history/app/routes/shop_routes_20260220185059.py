...
@login_required
@roles_required("admin", "manager")  # только папа/manager/admin передают товары
def transfer_to_shop():
    """Передача товара из фабрики в магазин."""
    factory_id = current_user.factory_id

    if request.method == "POST":
        # --- чтение и валидация формы ---
        try:
    ...
    return render_template("shop/transfer.html", products=products)
...
def export_shop():
    ...
    csv_text = shop_service.export_items_csv(
        q=q or None,
        sort=sort,
        factory_id=current_user.factory_id,
    )
    ...
    return Response(
        csv_text,
        mimetype="text/csv",
        headers={"Content-Disposition": "attachment; filename=shop_stock.csv"},
    )
