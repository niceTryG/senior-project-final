...
from ..models import CashRecord, ShopOrder, Production, Product
...
def _get_production_today_summary(factory_id: int):
    """
    Dashboard helper:
    - produced_today_total: total pieces produced today
    - produced_today_models: number of models produced today
    - prod_today_rows: list of (product_name, qty_sum) sorted by qty desc
    """
    today = date.today()
...
def _build_admin_dashboard_data(factory_id: int):
    ...
    stats = {
        ...
        # ---- production (existing service stats) ----
...
def dashboard():
    ...
    # NEW: production today summary (for dashboard card)
    ...
    if role == "shop" or current_user.is_shop:
        ...
        # NEW vars for dashboard_manager.html
        data["produced_today_total"] = produced_today_total
        data["produced_today_models"] = produced_today_models
        data["prod_today_rows"] = prod_today_rows
        ...
        # NEW vars for dashboard.html too
        data["produced_today_total"] = produced_today_total
        data["produced_today_models"] = produced_today_models
        data["prod_today_rows"] = prod_today_rows
