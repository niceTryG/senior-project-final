...
def produce_today_save():
    ...
    FIXED:
    - On laptop you can have duplicate qty_<id> inputs (mobile + desktop in same DOM).
      request.form.items() picks the FIRST value (often empty) -> "nothing saved".
      Now we correctly take the LAST non-empty value.
    """
    factory_id = current_user.factory_id
    products = _get_products_for_factory(factory_id)
    ...
    # ✅ IMPORTANT FIX: use .lists() to get ALL values per key
    for key, values in request.form.lists():
        ...
        # Take LAST non-empty value (desktop input on laptop usually comes last)
        qty = 0
