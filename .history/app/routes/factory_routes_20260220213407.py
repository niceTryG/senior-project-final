...
def produce_today():
    ...
    # smart partial search (latin/cyrillic tolerant)
    if q:
        variants = _q_variants(q)
        like_filters = []
        for v in variants:
    ...
    # categories list for dropdown (only this factory)
    categories = [
        c[0] for c in db.session.query(Product.category)
        .filter(Product.factory_id == factory_id, Product.category.isnot(None), Product.category != "")
        .distinct()
        .order_by(Product.category.asc())
        .all()
    ]
...
def api_product_suggest():
    ...
    candidates = base_query.order_by(Product.name.asc()).limit(80).all()
