...
def _import_dad_sales_excel(df: pd.DataFrame, factory_id: int):
    ...
    IMPORTANT:
      - Your ShopStock model has NO factory_id, so we query ShopStock only by product_id.
      - We sell only what exists in shop stock.
    """
    df = df.copy()
    df.columns = [str(c).strip().lower() for c in df.columns]
    ...
    try:
        for _, row in df.iterrows():
            ...
            # Product is factory-scoped (good)
            ...
            # ✅ ShopStock has no factory_id -> query ONLY by product_id
            ...
            # Create sale (adjust field names if your Sale differs)
            sale = Sale(
                factory_id=factory_id,
                product_id=product.id,
                quantity=sell_qty,          # if your field is "qty", change to qty=sell_qty
                price_uzs=price,            # adjust if needed
                total_uzs=sell_qty * price, # adjust if needed
                sold_at=sold_at,
        ...
        if missing_qty_total > 0:
            flash(
                ...
                f"⚠️ Missing shop stock total qty: {missing_qty_total} (not imported).",
                "warning",
            )
        else:
