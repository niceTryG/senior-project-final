...
from ..models import Product, ShopStock, Sale  # Sale must exist in your models
...
def _ensure_factory_bound():
    ...
    Returns factory_id (int) or None.
    If invalid for normal user → flashes and returns None.
    """
...
def _safe_datetime(value) -> datetime:
    """Parse Excel date / string date safely. Fallback = now UTC."""
    try:
...
def import_products():
    ...
    # Read file bytes once (important: we will read multiple sheets/headers)
    try:
    ...
    # -----------------------------
    # 1) Try to detect Dad Excel table "Реализация"
    #    We look for a header row that contains "модель" and "сони"
    # -----------------------------
    try:
        ...
        for sheet in xls.sheet_names:
            ...
            for i in range(min(60, len(preview))):
                row_text = " ".join([c.strip().lower() for c in preview.iloc[i].tolist()])
                if ("модель" in row_text) and (("сони" in row_text) or ("soni" in row_text)):
                    header_row = i
                    break
            ...
            # Re-read the sheet using found header row
            ...
            # Dad Реализация usually also has these:
            ...
            if ("модель" in candidate.columns) and (("сони" in candidate.columns) or ("soni" in candidate.columns)) and has_price and has_date:
                dad_df = candidate
                dad_sheet = sheet
                dad_header_row = header_row
                break

        if dad_df is not None:
            # ✅ Dad file detected -> import as sales using your existing helper
            # (Your helper expects df with columns like модель/сони/нархи/число)
            return _import_dad_sales_excel(dad_df, factory_id)

    except Exception as e:
        # If dad detection fails, we still continue with normal import
        # (but we show info so you know why)
        flash(f"Note: Dad Excel auto-detect failed, trying normal import. Reason: {e}", "warning")

    # -----------------------------
    # 2) Normal product import (your old logic)
    # -----------------------------
    try:
    ...
    col_map = {
        "name": ["name", "model", "модель", "номи", "наименование"],
...
def _import_dad_sales_excel(df, factory_id: int):
    """
    Import Dad Excel 'Реализация' as SALES.
    Columns (expected): число | модель | сони | нархи | сумма
    Notes:
      - We do NOT create ShopOrder here (your ShopOrder schema doesn't support product/qty).
      - If shop stock is not enough, we sell only what exists and count "missing".
    """
    name_col = "модель"
    ...
    try:
        for _, row in df.iterrows():
            ...
            if sell_qty < qty:
                missing_qty_total += (qty - sell_qty)

            # If no stock -> don't create sale
            if sell_qty <= 0:
                continue
            ...
            sale = Sale(
                ...
                quantity=sell_qty,      # adjust if your Sale uses qty instead of quantity
                price_uzs=price,        # adjust field names if different
        ...
        if missing_qty_total > 0:
            flash(
                f"Dad Excel imported ✅ Sales: {sales_created}. "
                f"⚠️ Not enough shop stock for total missing qty: {missing_qty_total} (not imported).",
                "warning",
            )
        else:
            flash(f"Dad Excel imported ✅ Sales: {sales_created}", "success")
...
def product_cost(product_id: int):
    ...
    if request.method == "POST":
        ...
        # Your model might use cost_price_per_item or similar; keep as you had it
