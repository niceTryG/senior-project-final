...
class Product(db.Model):
    ...
    def stock_value_cost(self) -> float:
        """Total cost value of current factory stock for this product."""
        return self.quantity * (self.cost_price_per_item or 0.0)

    def stock_value_sell(self) -> float:
        """Total potential revenue if all factory stock is sold at current selling price."""
        return self.quantity * (self.sell_price_per_item or 0.0)

    def stock_profit(self) -> float:
        """Potential profit in current factory stock."""
...
class ShopStock(db.Model):
    ...
    @property
    def total_value(self) -> float:
        """Total money in shop for this product (qty * sell price)."""
...
class ShopOrder(db.Model):
    ...
    def recalc_status(self) -> None:
        """
        Recalculate order status based on remaining quantities.

        - if status is already completed/cancelled → do nothing
        - if no items → pending
        - if all items have qty_remaining <= 0 → ready
        - else → pending
        """
...
class StockMovement(db.Model):
    ...
    # quantity change (+ incoming, - outgoing)
    qty_change = db.Column(db.Integer, nullable=False)

    source = db.Column(db.String(50))
    destination = db.Column(db.String(50))

    # movement_type: factory_to_shop, shop_sale, adjustment, ...
...
class Movement(db.Model):
    ...
    source = db.Column(db.String(64), nullable=False)       # factory, shop, customer
    destination = db.Column(db.String(64), nullable=False)  # shop, customer, factory, order
    change = db.Column(db.Integer, nullable=False)          # +100, -20 etc
    note = db.Column(db.String(255))
    created_by_id = db.Column(db.Integer, db.ForeignKey("users.id"))
    timestamp = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)
...
class ExcelImportRow(db.Model):
    ...
    factory_id = db.Column(db.Integer, db.ForeignKey("factories.id"), nullable=False)
    kind = db.Column(db.String(32), nullable=False)  # "sale" or "cash"
    row_hash = db.Column(db.String(64), nullable=False)  # sha256 hex
    ...
    def __repr__(self):
        return f"<ExcelImportRow factory_id={self.factory_id} kind={self.kind} row_hash={self.row_hash}>"
