...
def product_cmd(update: Update, context: CallbackContext) -> None:
    ...
    if not args:
        update.message.reply_text(
            "Использование:\n"
            "• /product bodik 56\n"
            "• /product kostyum zvezda\n"
            "• /product 12  (по внутреннему ID, если нужно)\n"
        )
        return
    ...
    # ----- 1) Попробуем ID (если это чистое число или MM-00012) -----
    matches = []
    pid = None
    raw = query_str.upper()
    ...
    if pid is not None:
        p = base_q.filter(Product.id == pid).first()
        if p:
            matches = [p]

    # ----- 2) Если по ID ничего не нашли — ищем по имени (LIKE) -----
    if not matches:
        matches = (
            base_q
            .filter(Product.name.ilike(f"%{query_str}%"))
    ...
    # ----- 3) Если много совпадений — показать список, без деталей -----
    if len(matches) > 1:
        lines = [f"Найдено {len(matches)} моделей по запросу “{query_str}”:\n"]
        for p in matches:
            lines.append(
                f"• {p.name} ({p.category or '-'}) — код MM-{p.id:05d}"
            )
        lines.append("\nМожно уточнить запрос, например: /product bodik 56")
        update.message.reply_text("\n".join(lines))
        return

    # ----- 4) Ровно один товар — показываем подробную карточку -----
    product = matches[0]

    factory_qty = product.quantity or 0  # фабричный склад
